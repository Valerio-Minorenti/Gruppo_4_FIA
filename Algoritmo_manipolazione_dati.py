...
#PASSO 0
def salva_percorso(input_path, output_path):
    """
   salva i percorsi e poi li dice 

    args:
        input_path (str): oil percorso del file di input
        output_path (str): percorso dell'output

    return:
        il percorso input e output
    """
    print(f"Percorso del file di input: {input_path}")
    print(f"Percorso del file di output: {output_path}")
    return input_path, output_path


#Passo 0.5
def carica_file(input_path):
    """
    il file tabulare lo si carica come df

    Args:
        input_path (str): percorso dell'input

    Returns:
        pd.DataFrame: df con i dati
    """
    ext = os.path.splitext(input_path)[1].lower()  # estrae l'estensione del file
...
def salva_file(df, output_path):
    ...
    Returns:
        None
    """
    ext = os.path.splitext(output_path)[1].lower()  # estrae l'estensione
    
    try:
        if ext == ".csv":
            df.to_csv(output_path, index=False)
        elif ext == ".xlsx":
            df.to_excel(output_path, index=False, engine='openpyxl')  # engine
        elif ext == ".tsv":
            df.to_csv(output_path, sep='\t', index=False)
        else:
            print(f"attenzione! l'estensione {ext} non è supportata, aggiungila")
            return
        print(f"Fìile salvato con successo in: {output_path}")
    except Exception as e:
